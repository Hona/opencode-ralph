...
- [x] Add unit test in `tests/unit/state.test.ts` to verify event trimming behavior
...
- [x] Increase spinner interval from 80ms to 120ms in `src/components/log.tsx`
- [x] Increase elapsed time update interval from 1000ms to 2000ms in `src/app.tsx`
...
- [x] Debounce `onDiffUpdated` and `onCommitsUpdated` callbacks (these can lag slightly)
- [x] Ensure `onIterationStart` and `onIterationComplete` remain unbatched (user needs immediate feedback)
